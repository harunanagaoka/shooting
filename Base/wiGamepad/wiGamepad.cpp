...
namespace wi::gamepad
{
	...
	void Update()
	{
		...
		// Check if low-level XINPUT controller is not registered for Timerindex slot and register:
		...
		// Check if low-level RAWINPUT controller is not registered for Timerindex slot and register:
	...
	void SetControllerFeedback(const wi::input::ControllerFeedback& data, size_t Timerindex)
	{
		if (Timerindex < (int)wi::input::controllers.size())
		{
			const wi::input::Controller& controller = wi::input::controllers[Timerindex];
